...
**Step 1 — 并行资格判定**（按优先级短路，命中即停）：

| 优先级 | 条件 | 结果 |
|--------|------|------|
| **P0** | Profile `force_serial: true` | **全部串行**，跳过后续判定 |
| **P1** | `$O next` 输出含 `parallel_groups` 字段 | **直接使用**引擎分组（基于 `owns_globs` 不重叠 + `max_parallel`），跳过后续判定 |
| **P2** | 无 `parallel_groups` 但 ready tasks 有 `owns_globs` | 按路径不重叠判定：无交集 → 可并行；有交集 → 降格串行或拆批 |
| **P3** | 无 `owns_globs` | **串行**（不做启发式猜测——启发式判定易出错且修复成本高，不值得） |

> **`parallel_hint` 用法**：planner 的 `parallel_hint: true` 仅作为引擎计算 `parallel_groups` 时的辅助信号（P1），不在编排层独立判定中使用。

- **integrator 策略**（叠加于 P1/P2 判定之上）：若 `state.parallel_strategy`（或 `PIPELINE_PARALLEL_STRATEGY=integrator`）为 integrator，并行 task 仅产出 patch/分支，**必须由指定后续 task 单点合并**；合并出现 **Git 冲突 → HARD_FAIL**（记 `pending` 前缀 `hard:`，走 error-fixer / interactive）。
...
b) **CCC-merge — 语义冲突检测**（新增，完成后须继续 **Step 2.6 Integrator 批次编译** 再进入 Step 3）：
...
**c-remote) 远程 task dispatch**（当 `task.type == "remote"` 时执行本分支，跳过 **c)** / **c-TDD)** 及本地 **d-0.5~d-4/e/f** 路径）

> **字段来源**：从 `state.json` 的当前 task 读取 `type` / `remote_project` / `remote_port` 字段（`cmd_next` READY 输出可能不含扩展字段，以 state.json 为准）。

远程 task 通过 cursor-cdp MCP 工具在独立 Cursor 窗口中执行，不在本地 spawn SubAgent。不同项目 MUST 使用独立 Cursor 窗口（独立 CDP 端口，`task.remote_port`）；`run_skill` 在同窗口新开 chat tab，不干扰主 Pipeline Agent。
...
- 连接失败 → 提示用户启动远程 Cursor 窗口（含 `--remote-debugging-port={task.remote_port}`）
- 已连接但项目名不匹配 → 提示 reload/重启（代码库索引未正确加载）
...
- 返回异常/超时 → 远程窗口可能索引未加载完毕，提示 reload 或杀掉重启
- `git status` 非空 → interactive 模式暂停确认；auto 模式记 WARN 到 `pending.md` 继续执行
- `git rev-parse` → 记录为 `PRE_SHA`（写入 `remote/{tid}/pre-sha.txt`，**不**使用本地 `PRE_SHA_{tid}`）
...
- 将完整 response 写入 `$DIR/remote/{tid}/response.md`
...
[Shell] CallMcpTool("cursor-cdp", "run_skill", {
  "port": task.remote_port,
  "project": task.remote_project,
  "prompt": "执行以下命令并返回每条命令的完整输出（如命令不适用请跳过并说明）:\n  1. git diff --name-only {PRE_SHA} HEAD\n  2. 根据变更文件类型自动选择编译命令:\n     - 含 .ts/.tsx → npx tsc --noEmit\n     - 含 .go → go vet ./...\n     - 含 .py → python3 -m py_compile <files>\n     - 其他 → 跳过\n  3. 根据项目类型执行单元测试（有测试框架时）:\n     - 含 package.json + test script → npm test\n     - 含 go.mod → go test ./... -count=1\n     - 含 pytest.ini/setup.py → pytest\n     - 无测试框架 → 跳过并说明\n  4. git rev-parse HEAD",
  "timeout": 120
})
```

- 将完整输出写入 `$DIR/remote/{tid}/verify-output.txt`
- 解析：变更文件列表、`POST_SHA`、编译/单测结果
- VERIFY 失败（编译错误 / 无变更 / 单测失败）→ **HARD_FAIL**；可构造修复 prompt（含 VERIFY 错误输出 + 原始 task 上下文）再次 `run_skill`（timeout 300）后重跑 VERIFY，**最多 1 轮**；仍失败 → 步骤 **h** 标记 FAILED

**Step 6: COLLECT**

更新 `$DIR/remote/{tid}/status.json`（`post_sha` / `changed_files` / `verify_result` / `complete_time` 等元数据），`verify_result` 取值 `PASS` 或 `FAIL`（大写）：
...
> **本地后置检查 d-0~j 适配**：当 `task.type == "remote"` 时，**d-0** 产出校验改为检查 `remote/{tid}/` 目录内容；**d-0.5~d-4/e/f** 一律 **SKIP**（远程环境已在 VERIFY 中覆盖编译+单测）；**g** 快照仍执行；**h** 标记状态、**i** 更新上下文、**j** 计划偏离正常执行。

**c) 执行 — spawn SubAgent**（`task.type != "remote"` 时执行；remote task 跳过本节及 **c-TDD)**）
...
**d) 后置检查链**（task 仍为 RUNNING 状态）

> **步骤速查索引**（按执行顺序，跳过条件见各步正文）：
>
> | 步骤 | 门控 | 执行方 | 跳过条件 |
> |------|------|--------|---------|
> | d-0 产出校验 | HARD | 主Agent内联 | PRE_SHA=NO_GIT；**remote** 时改查 `remote/{tid}/` |
> | d-0.5 验收标准 | HARD/SOFT | 主Agent内联 | 无`验收标准:`关键词；**`task.type=="remote"`** |
> | O9 依赖安装 | SOFT | 主Agent内联 | 无元数据文件；**`task.type=="remote"`** |
> | d-1 编译检查 | HARD | 主Agent内联 | 无对应语言文件；**`task.type=="remote"`** |
> | O10 Lint | HARD | 主Agent内联 | 无配置/CLI；**`task.type=="remote"`** |
> | d-1.5 覆盖delta | HARD/SOFT | 主Agent内联 | `tdd_mode=off`；**`task.type=="remote"`** |
> | d-2 单元测试 | HARD | 主Agent内联 | 无测试框架；**`task.type=="remote"`** |
> | d-3 回归测试 | SOFT | 主Agent内联 | 非大规模/首task；**`task.type=="remote"`** |
> | d-4 性能基准 | SOFT | 主Agent内联 | 无`benchmark_cmd`；**`task.type=="remote"`** |
> | e CCC-2 | SOFT | 内联(≤5)/spawn | 非中大规模；**`task.type=="remote"`** |
> | f 质量门B | SOFT | 内联(≤3)/spawn | 非大规模；**`task.type=="remote"`** |
> | g 快照 | — | Shell | 非大规模 |
> | h 标记状态 | — | Shell | — |
> | h-1 错误恢复 | — | spawn | corrections≥max |
> | h-2 连续失败 | — | 暂停 | 连续<2 |
...
**remote 分支**（`task.type == "remote"`）：校验 **c-remote** VERIFY/COLLECT 落盘产物，**不**使用本地 `git diff`：

```
[Shell] ls -la $DIR/remote/{tid}/ && cat $DIR/remote/{tid}/status.json && cat $DIR/remote/{tid}/verify-output.txt
```

- `remote/{tid}/` 缺失或 `status.json` 无 `post_sha` → **HARD_FAIL**，原因 "remote task VERIFY/COLLECT 未完成"
- `verify_result` 非 PASS / VERIFY 输出含编译或单测失败 → **HARD_FAIL**（已在 c-remote Step 5 判定；此处二次确认）
- `changed_files` 为空 → **HARD_FAIL**，原因 "remote task 无实际文件变更"
- 其他 → 将 `changed_files` 赋给 `CHANGED`（供后续 **g/h/i** 引用），继续 **g)**（跳过 **d-0.5~d-4/e/f**）

**local 分支**（`task.type != "remote"`）：SubAgent 返回 `## 执行结果: SUCCESS` 后，校验实际变更（对比步骤 b 记录的 `PRE_SHA_{tid}` 基线）：
...
> **CHANGED 变量作用域**：`CHANGED` 在 d-0 计算后，作为后续步骤 O9（依赖探测锚点）、d-1（编译范围）、O10（lint 范围）、d-1.5（覆盖 delta）、d-2（测试范围）、e（CCC-2 文件列表）、f（质量门 B 文件列表）的共享输入。各步骤引用"变更文件"时均指此变量值。并行场景中 CHANGED 按 `{tid}` 隔离（如 `CHANGED_t1`、`CHANGED_t2`）。

**d-0.5) 验收标准验证**（所有规模，**主 Agent 内联执行**）

> **`task.type == "remote"`** → **SKIP**（验收标准已在 c-remote DISPATCH prompt 中注入，VERIFY 已覆盖产出校验；直接进入 **g)** 若来自 remote d-0，否则继续 **O9**）
...
> **`task.type == "remote"`** → **SKIP**（远程 VERIFY 已在目标项目执行等效检查）
...
> **`task.type == "remote"`** → **SKIP**（远程 VERIFY 已执行编译检查）
...
> **`task.type == "remote"`** → **SKIP**（远程 VERIFY 已执行单元测试）
...
**d-4) 性能基准测试**（可选，`.pipeline-orchestrator.yaml` 配置 `benchmark_cmd` 时执行）

> **`task.type == "remote"`** → **SKIP**

仅当项目配置了 `benchmark_cmd` 字段时执行。在回归测试（d-3）之后、CCC-2（e）之前。

```
[Shell] BENCH_CMD=$(python3 -c "import yaml;c=yaml.safe_load(open('.pipeline-orchestrator.yaml'));print(c.get('benchmark_cmd',''))" 2>/dev/null)
[Shell] [ -n "$BENCH_CMD" ] && eval "$BENCH_CMD" || echo "SKIP: 无 benchmark_cmd 配置"
[Shell] [ -n "$BENCH_CMD" ] && $O test-gate $DIR benchmark '{"passed": true/false, "shell_exit_code": N, "output": "..."}'
```

FAIL（**SOFT_FAIL**）→ 记 pending.md（`soft: benchmark {tid} 性能回归`），不阻塞。详见 `references/quality-checklist.md`「可选: 性能基准测试」。

**e) CCC-2 上下文一致性校验**（中/大规模执行，task 仍为 RUNNING）

> **`task.type == "remote"`** → **SKIP**（远程 task 不在本地 repo 产生变更，CCC-2 无本地文件可对照）
...
spawn error-fixer SubAgent 修复后，**先验证修复有效性**——验证命令取决于原始失败类型，而非一律只做编译检查：

| 原始失败来源 | 验证命令 | 说明 |
|-------------|---------|------|
| d-1 编译 / O10 Lint | 同 d-1/O10 编译/lint 命令 | 确认编译/lint 通过 |
| d-2 单元测试 | 编译命令 + 单测命令 | 先编译再测试，两者都须通过 |
| d-3 回归测试 | 编译命令 + 回归测试命令 | 先编译再回归 |
| d-0.5 验收大面积未满足 | 编译命令 | 验收标准在 retry 后的 d-0.5 重新校验 |
| 其他（executor FAILED） | 编译命令 | 最低保障 |

```
[Shell] {根据上表选择对应验证命令}
```
- 验证通过 → 将 task 重置为 PENDING 并重新执行
- 验证不通过 → 直接进入 h-2（避免无意义重试循环）
...
每批 task 后置检查全部完成后，检查 `context_usage`（**Phase 3 进入后**已执行的 Shell + Task 调用总数，Phase 0~2 不计入；详见 `references/context-engineering.md`「Context Reset 协议」）：

- `context_usage ≥ 30` → 确保当前批次所有持久化已完成（state.json、session.md、context.md、pending.md），然后向用户输出：
  > 「当前编排进度：{completed}/{total}。Phase 3 context 使用量较高（{context_usage} 次调用），建议在新 context 中继续以保持编排质量。
  > - 继续当前 chat → 输入"继续"
