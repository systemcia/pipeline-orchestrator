...
注入预算（O8）：
  task 规模（小/中/大）× depends_on 基数 → 动态夹逼
  小 task：800~1400 字符    中 task：1000~2400 字符
  大 task：1200~3000 字符   通用约束永不裁剪
```
...
两个层级自动触发：CCC-2（代码 vs task 设计对齐）、CCC-merge（并行 task 合并语义冲突检测）。提案阶段由独立 evaluator 按 PA01-PA14 清单逐条审查覆盖率和术语一致性。
...
Phase 0 自动判定规模，小需求跳过重流程，大需求全保障。四个内置 Profile（default/small/hotfix/thorough）覆盖日常到紧急修复，支持自定义模板扩展。

## TDD 纪律集成

三级 `tdd_mode`（off/prompt/strict）：prompt 模式软引导测试先行；strict 模式强制 RED-GREEN-REFACTOR 循环，无测试文件则 HARD_FAIL。配合 `systematic-debugging` + `verification-guard` 形成 Superpower 三件套。

## 状态引擎：断点续传 + 回滚

`$O` CLI（22 个子命令）管理所有状态，文件锁保证原子性，中断后自动从 `state.json` 恢复。
...
7. **测试纪律可选升级** — 从 off 到 strict，渐进式引入 TDD
