...
- **搜索范围**：`inject-rag` 默认搜索 `session-analysis` 类内容（管理台 RAG 端点 `scope=session-analysis`）；SubAgent prompt 中注入的 RAG 结果应标注来源章节（`[经验总结]` / `[改进建议]`），便于区分。
...
| 管理台 API 不可用 | RAG 回退到本地 session-analysis.md 文件搜索；趋势跳过，不阻塞 | 0 |
| 编译检查 FAIL | 主 Agent 内联执行；FAIL → spawn error-fixer + 内联重试 **1 次**（d-1 步骤内级重试）。仍 FAIL → 标记 task FAILED → h-1 task 级 retry（受 `max_task_retries` 约束） | 步骤内 1 + task 级 `max_task_retries`（默认 1） |
| 单元测试 FAIL | 同编译检查：d-2 步骤内 spawn error-fixer + 重试 1 次。仍 FAIL → h-1 task 级 retry | 步骤内 1 + task 级 `max_task_retries`（默认 1） |
| E2E 测试 FAIL | **SOFT_FAIL**：记 pending.md（`soft:` 前缀），不阻塞（E2E 依赖外部环境，前序门已保障代码质量） | 0 |
| CCC 偏离 | 小型 task（≤5 文件）主 Agent 内联自检；大型 task spawn SubAgent；偏离记 pending.md，不阻塞 | 0 |
