...
- 用户选继续 → 从 `$O list` 输出中取对应 session 的 `dir` 字段作为 `$DIR`，读取 `$DIR/state.json` 获取 `scale` 确定规模裁剪、`profile` 确定编排 profile（无则按 scale 匹配默认 profile）、`openspec_change` 确定是否为 OpenSpec 模式，读取 `$DIR/session.md` 恢复上下文。**如有 RUNNING 状态的 task**（上次中断遗留），在 Phase 3 的 3.1 步骤中会自动处理（过期清理）
