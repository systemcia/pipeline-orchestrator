...
```
 ...
 3.2 并行资格判定     过期清理           │
  ...
  └─ 串行 ──→ 逐个 a→b→c ──┐  │         │
                             ▼  ▼         │
                3.3 逐个后置检查 d-0→O9→d-1→O10→d-2~i ────┘
                             │
                             ▼ ALL_DONE
                           Phase 4
```
...
管理台不可用时，回退到**本地文件搜索**：
```
[Shell] grep -rl "<关键词>" /opt/pipeline-orchestrator/sessions/*/lessons.md /opt/pipeline-orchestrator/sessions/*/improvements.md 2>/dev/null | head -3 | xargs -I{} head -20 {} 2>/dev/null || echo "INFO: 无本地历史经验"
```
...
- `PRE_SHA_{tid}=NO_GIT` → 跳过校验，继续 **O9**（无 CHANGED 列表时按 **O9**「无 CHANGED 列表」条处理）
- CHANGED 为空（未修改/新增任何文件）→ **HARD_FAIL**，原因 "executor 声称 SUCCESS 但无实际文件变更"
- CHANGED 包含明显与 task 无关的文件（如修改了 session.md 或编排层文件）→ 记 pending（soft: 产出范围可能超出 task 描述），继续
- 其他 → 通过，继续 **O9**
...
|--------|---------------------------|
| `go.mod` | `go mod tidy` |
| `package.json` | 同目录存在 `yarn.lock` → `yarn install --frozen-lockfile`（或项目文档约定的等价冻结安装）；否则存在 `package-lock.json` 或 `npm-shrinkwrap.json` → `npm ci`；**仅** `package.json` 而无上述 lock 文件 → **SKIP+INFO**（不写为失败） |
...
```
然后回到 3.1（`$O next` 会重新将该 task 列为 READY）。**每个 task 最多重试 1 次**（corrections=1 时可重试，corrections >= 2 时 `$O retry` 会输出 WARN，应放弃重试进入 h-2）。
