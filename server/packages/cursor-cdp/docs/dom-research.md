...
> **调研日期**: 2026-06-06  
> **验证状态**: 本文档基于公开资料与社区逆向项目汇总；**本地 CDP 端点未连通**（`9222`/`9226` 均不可用），部分选择器标注为「推测」，需在后续 task 中通过 `opencli cursor dump` 或 CDP `Runtime.evaluate` 实测确认。
...
| CDP 端口 | `--remote-debugging-port=9226`（本项目约定；社区常用 `9222`） |
...
# 2. 检查 CDP 端点
curl http://127.0.0.1:9226/json/list | jq '.[].url'
