...
```
$O init              <name> <tasks_json> [profile]  创建 session
$O list                                           列出所有 session
...
| Phase 3 RAG 注入 | `rag-inject` | 跳过 | ✓ | ✓ |
| Phase 3 产出校验 | — | ✓ | ✓ | ✓ |
| Phase 3 编译检查 | `compile` | ✓ | ✓ | ✓ |
| Phase 3 单元测试 | `unit-test` | 跳过 | ✓* | ✓* |
...
| Phase 4 质量门 C + 总结 | — | 跳过 | ✓ | ✓ |
| Phase 4 完成+归档 | — | ✓ | ✓ | ✓ |
| Phase 5 经验总结与改进建议(5a，含 improvements.md) | — | 跳过 | ✓ | ✓ |
| Phase 5 趋势(5b) | — | 跳过 | 跳过 | ✓ |
...
- 需求确认 → 提案自动生成 → 质量门自检 → **停：用户审批提案**（Phase 1g 为唯一必停点）
- 执行自动推进 → RAG 注入 → 测试门 → 质量门
- 失败自动重试 1 次后标记 FAILED，下游依赖 task 由 `$O next` 自动级联标记 SKIPPED（原因 "dependency failed"）
- 所有自动决策写 `$DIR/pending.md`
- 完成后自动 Phase 5（中规模执行 5a，大规模执行 5a+5b）
- **最终统一呈现所有 pending 决策，由用户确认**
