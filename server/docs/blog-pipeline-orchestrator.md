...
管理台的趋势分析页面实时展示编排效能指标，以下为典型观测维度：

| 指标 | 来源 | 说明 |
|------|------|------|
| 累计编排 Session | `/api/analytics/overview` | 自动统计 |
| task 一次通过率 | 趋势分析 | 衡量 SubAgent 质量 |
| 经验命中率（RAG） | RAG 搜索日志 | 衡量知识复用效果 |
| 质量门拦截率 | 趋势分析 | 高拦截 = 早发现真问题 |
| 编排覆盖代码行 | Session 汇总 | 自动从 git diff 统计 |
